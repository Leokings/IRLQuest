...
Production requires a Google-authenticated explorer. Local development still starts with a demo explorer so the full vertical slice can be tested without cloud credentials. A verifier is deployed on Testnet Bradbury at [`0xec9569A41A715D962DD24b3be623792D08a7e709`](https://explorer-bradbury.genlayer.com/address/0xec9569A41A715D962DD24b3be623792D08a7e709); its byte-for-byte verified deployment details are in [`deployments/bradbury.json`](./deployments/bradbury.json).
...
- **Verdict:** GenLayer intelligent-contract verification on Bradbury in production, with an explicitly labelled local demo verifier for development
...
1. Give the configured relayer account enough GEN to submit verification transactions.
2. Host the app at a validator-reachable HTTPS URL.
3. Copy `.env.example` to `.env`, set the public URL, and override any deployment value if needed.
...
npm run deploy:bradbury # Deploy and byte-verify once; refuses duplicate deployment records
